...
enum Precedence {
    PREC_MIN, // minimum precedence (for default at the start of subexpression
              // parsing)
    PREC_ASSIGNMENT,
    PREC_OR,
    PREC_AND,
    ...
    PREC_MULTIPLICATIVE,
    PREC_UNARY,
    PREC_PRIMARY // for identifiers, function calls, literals and parenthesized
                 // expressions
};
...
inline void print_ast_expression(AST_Expression *ast_expr,
...
inline std::string get_leading_whitespace(std::string line) {
    ...
    std::string leading_ws =
        (pos != std::string::npos) ? line.substr(0, pos) : line;
    return leading_ws;
}
