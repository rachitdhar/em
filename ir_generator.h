...
inline llvm::Value *cast_llvm_value_to_bool(llvm::Value *val,
                                            llvm::LLVMContext &_context,
...
inline llvm::Value *generate_ir__logical_and(AST_Expression *left,
                                             AST_Expression *right,
                                             LLVM_IR *ir) {
    llvm::BasicBlock *current_block = ir->_builder->GetInsertBlock();
    llvm::Function *f = current_block->getParent();

    llvm::BasicBlock *andright =
        llvm::BasicBlock::Create(ir->_context, "andright", f);
    llvm::BasicBlock *andend =
        llvm::BasicBlock::Create(ir->_context, "andend", f);
    ...
    llvm::PHINode *phi_node = ir->_builder->CreatePHI(
        llvm::Type::getInt1Ty(ir->_context), 2, "andtmp");
    phi_node->addIncoming(llvm::ConstantInt::getFalse(ir->_context),
                          current_block);
...
inline llvm::Value *generate_ir__logical_or(AST_Expression *left,
                                            AST_Expression *right,
                                            LLVM_IR *ir) {
    llvm::BasicBlock *current_block = ir->_builder->GetInsertBlock();
    llvm::Function *f = current_block->getParent();

    llvm::BasicBlock *orright =
        llvm::BasicBlock::Create(ir->_context, "orright", f);
    llvm::BasicBlock *orend =
        llvm::BasicBlock::Create(ir->_context, "orend", f);
    ...
    llvm::PHINode *phi_node = ir->_builder->CreatePHI(
        llvm::Type::getInt1Ty(ir->_context), 2, "ortmp");
    phi_node->addIncoming(llvm::ConstantInt::getTrue(ir->_context),
                          current_block);
...
inline bool generate_block_ir(LLVM_IR *ir,
