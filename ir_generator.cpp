...
llvm::Value *AST_Literal::generate_ir(LLVM_IR *ir) {
    ...
    case T_STRING: {
        ...
        return ir->_builder->CreateGlobalString(
...
llvm::Value *AST_Function_Definition::generate_ir(LLVM_IR *ir) {
    ...
    llvm::BasicBlock *function_entry =
        llvm::BasicBlock::Create(ir->_context, "entry", _f);
    ...
    for (auto &arg : _f->args()) {
        ...
        llvm::AllocaInst *_alloca =
            tmp_builder.CreateAlloca(arg.getType(), nullptr, param_name);
...
llvm::Value *AST_If_Expression::generate_ir(LLVM_IR *ir) {
    ...
    _condition = ir->_builder->CreateICmpNE(
        _condition, llvm::ConstantInt::get(_condition->getType(), 0), "ifcond");
    ...
    llvm::BasicBlock *_then =
        llvm::BasicBlock::Create(ir->_context, "then", _f);
    llvm::BasicBlock *_else =
        llvm::BasicBlock::Create(ir->_context, "else", _f);
    llvm::BasicBlock *_ifend =
        llvm::BasicBlock::Create(ir->_context, "ifend", _f);
...
llvm::Value *AST_For_Expression::generate_ir(LLVM_IR *ir) {
    ...
    llvm::BasicBlock *_forcond =
        llvm::BasicBlock::Create(ir->_context, "forcond", f);
    llvm::BasicBlock *_forbody =
        llvm::BasicBlock::Create(ir->_context, "forbody", f);
    llvm::BasicBlock *_forinc =
        llvm::BasicBlock::Create(ir->_context, "forinc", f);
    llvm::BasicBlock *_forend =
        llvm::BasicBlock::Create(ir->_context, "forend", f);

    // jump to condition check
    ir->_builder->CreateBr(_forcond);

    ir->_builder->SetInsertPoint(_forcond);
    llvm::Value *_condition = condition ? condition->generate_ir(ir) : nullptr;

    if (_condition) {
        _condition = ir->_builder->CreateICmpNE(
            _condition, llvm::ConstantInt::get(_condition->getType(), 0),
            "forcond");
    } else {
...
llvm::Value *AST_While_Expression::generate_ir(LLVM_IR *ir) {
    ...
    llvm::BasicBlock *_whilecond =
        llvm::BasicBlock::Create(ir->_context, "whilecond", f);
    llvm::BasicBlock *_body =
        llvm::BasicBlock::Create(ir->_context, "whilebody", f);
    llvm::BasicBlock *_whileend =
        llvm::BasicBlock::Create(ir->_context, "whileend", f);
...
llvm::Value *AST_Declaration::generate_ir(LLVM_IR *ir) {
    ...
    llvm::AllocaInst *_alloca =
        tmp_builder.CreateAlloca(var_type, nullptr, variable_name);
...
llvm::Value *AST_Unary_Expression::generate_ir(LLVM_IR *ir) {
    ...
    case TOKEN_BIT_NOT: {
        ...
        llvm::Value *val = expr->generate_ir(ir);
        llvm::Value *all_ones =
            llvm::ConstantInt::get(val->getType(), -1, true);
    ...
    case TOKEN_DECREMENT: {
        ...
        llvm::Value *val_addr =
            ((AST_Identifier *)expr)->generate_ir_pointer(ir);
        llvm::Value *val = expr->generate_ir(ir);

        if (val && val->getType()->isPointerTy())
            val = ir->_builder->CreateLoad(
                llvm::dyn_cast<llvm::PointerType>(val->getType()), val);
...
llvm::Value *AST_Binary_Expression::generate_ir(LLVM_IR *ir) {
    ...
    */
    ...
    if (op == TOKEN_AND)
        return generate_ir__logical_and(left, right, ir);
    if (op == TOKEN_OR)
        return generate_ir__logical_or(left, right, ir);
    ...
    if (right)
        Rval = right->generate_ir(ir);

    if (!L)
        return Rval;
    else if (!Rval) {
        // we need to return the Lval here
        return (left->expr_type == EXPR_IDENT) ? left->generate_ir(ir) : L;
    }
    ...
    For Assignments:
        ...
        Or, a combination of some binary operation and assignment (like +=, -=,
    etc.)
    */

    if (op == TOKEN_ASSIGN) {
        if (Rval && Rval->getType()->isPointerTy())
            Rval = ir->_builder->CreateLoad(
                llvm::dyn_cast<llvm::PointerType>(Rval->getType()), Rval);
    ...
    if (Lval && Lval->getType()->isPointerTy())
        Lval = ir->_builder->CreateLoad(
            llvm::dyn_cast<llvm::PointerType>(Lval->getType()), Lval);
    if (Rval && Rval->getType()->isPointerTy())
        Rval = ir->_builder->CreateLoad(
            llvm::dyn_cast<llvm::PointerType>(Rval->getType()), Rval);
    ...
    switch (op) {
    case TOKEN_PLUSEQ:
        result = ir->_builder->CreateAdd(Lval, Rval, "addtmp");
        break;
    case TOKEN_MINUSEQ:
        result = ir->_builder->CreateSub(Lval, Rval, "subtmp");
        break;
    case TOKEN_MULTIPLYEQ:
        result = ir->_builder->CreateMul(Lval, Rval, "multmp");
        break;
    case TOKEN_DIVIDEEQ:
        result = ir->_builder->CreateSDiv(Lval, Rval, "divtmp");
        break;
    case TOKEN_MODEQ:
        result = ir->_builder->CreateSRem(Lval, Rval, "modtmp");
        break;
    case TOKEN_LSHIFT_EQ:
        result = ir->_builder->CreateShl(Lval, Rval, "lshtmp");
        break;
    case TOKEN_RSHIFT_EQ:
        result = ir->_builder->CreateAShr(Lval, Rval, "rshtmp");
        break;
    case TOKEN_ANDEQ:
        result = ir->_builder->CreateAnd(Lval, Rval, "andtmp");
        break;
    case TOKEN_OREQ:
        result = ir->_builder->CreateOr(Lval, Rval, "ortmp");
        break;
    ...
    case TOKEN_BIT_OREQ:
        ...
        break;
    case TOKEN_XOREQ:
        result = ir->_builder->CreateXor(Lval, Rval, "xortmp");
        break;
    default:
        break;
    }
    ...
    switch (op) {
    case TOKEN_PLUS:
        return ir->_builder->CreateAdd(Lval, Rval, "addtmp");
    case TOKEN_MINUS:
        return ir->_builder->CreateSub(Lval, Rval, "subtmp");
    case TOKEN_STAR:
        return ir->_builder->CreateMul(Lval, Rval, "multmp");
    case TOKEN_DIVIDE:
        return ir->_builder->CreateSDiv(Lval, Rval, "divtmp");
    case TOKEN_MOD:
        return ir->_builder->CreateSRem(Lval, Rval, "modtmp");
    case TOKEN_LESS:
        return ir->_builder->CreateICmpSLT(Lval, Rval, "cmptmp");
    case TOKEN_GREATER:
        return ir->_builder->CreateICmpSGT(Lval, Rval, "cmptmp");
    case TOKEN_LESSEQ:
        return ir->_builder->CreateICmpSLE(Lval, Rval, "cmptmp");
    case TOKEN_GREATEREQ:
        return ir->_builder->CreateICmpSGE(Lval, Rval, "cmptmp");
    case TOKEN_EQUAL:
        return ir->_builder->CreateICmpEQ(Lval, Rval, "cmptmp");
    case TOKEN_NOTEQ:
        return ir->_builder->CreateICmpNE(Lval, Rval, "cmptmp");
    case TOKEN_LSHIFT:
        return ir->_builder->CreateShl(Lval, Rval, "lshtmp");
    case TOKEN_RSHIFT:
        return ir->_builder->CreateAShr(Lval, Rval, "rshtmp");
    case TOKEN_BIT_OR:
        return ir->_builder->CreateOr(Lval, Rval, "ortmp");
    case TOKEN_XOR:
        return ir->_builder->CreateXor(Lval, Rval, "xortmp");
    case TOKEN_AMPERSAND:
        return ir->_builder->CreateAnd(Lval, Rval, "andtmp");
    default:
        break;
    }
...
llvm::Value *AST_Return_Expression::generate_ir(LLVM_IR *ir) {
    ...
    llvm::Type *retTy =
        ir->_builder->GetInsertBlock()->getParent()->getReturnType();
...
llvm::Value *AST_Jump_Expression::generate_ir(LLVM_IR *ir) {
    ...
    switch (jump_type) {
    case J_BREAK:
        ir->_builder->CreateBr(current->loop_end);
        break;
    case J_CONTINUE:
...
llvm::Value *generate_ir__global_declaration(LLVM_IR *ir,
                                             AST_Expression *expr) {
    ...
    case EXPR_DECL: {
        ...
        auto *global = new llvm::GlobalVariable(
            *(ir->_module), var_type, false, llvm::GlobalValue::ExternalLinkage,
            init, decl->variable_name);
...
LLVM_IR *emit_llvm_ir(std::vector<AST_Expression *> *ast,
    ...
    auto *_builder =
        new llvm::IRBuilder<>(*_context); // helper to generate instructions
